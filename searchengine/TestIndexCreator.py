...
class TestIndexCreator(unittest.TestCase):
    ...
    def test_index_file(self):
        ...
        expected_comment_list = frozenset([
            Comment(1767167970,
                    ...
                    'Tragic event', 0, ['tragic', 'event']),
            Comment(1766936418,
                    'http://en.people.cn/n/2015/0101/c90785-8830442.html',
                    'Wang Wei', '2015-01-01T06:24:06', None, 0, 0,
                    'Xi > TrumP', 127, ['xi', '>', 'trump']),
            Comment(1766866409,
                    'http://en.people.cn/n/2015/0101/c90785-8830442.html',
        ...
        self.assertTrue(
            filecmp.cmp(f'{self.test_directory}/index.csv',
                        f'{self.test_directory}/expected_index.csv'),
            'expected and created index.csv are different')

    def test_seek_list(self):
        expected_seek_list = \
            PrefixDict([('>', 0), ('charact', 12), ('event', 30), ('some', 44),
                        ('special', 59), ('tragic', 77), ('trump', 92),
                        ('xi', 108), ('§¸', 121), ('·', 136), ('…', 149)])
    ...
    def test_compressed_seek_list(self):
        ...
        expected_compressed_seek_list = \
            {'>': (0, 4), 'charact': (4, 3),
             'event': (7, 3), 'some': (10, 4), 'special': (14, 3),
             'tragic': (17, 3), 'trump': (20, 4), 'xi': (24, 4), '§¸': (28, 4),
             '·': (32, 4), '…': (36, 4)}
        self.assertDictEqual(self.index_creator.compressed_seek_list,
                             expected_compressed_seek_list)

    def test_compressed_index_file(self):
        self.assertTrue(os.path.isfile(
            f'{self.test_directory}/expected_compressed_index'),
            'could not find expected_compressed_index file in '
            f'{self.test_directory}')
        self.index_creator.huffman_compression()
        self.assertTrue(
            os.path.isfile(f'{self.test_directory}/compressed_index'),
            'compressed index file was not created')
        self.assertTrue(
            filecmp.cmp(f'{self.test_directory}/compressed_index',
                        f'{self.test_directory}/expected_compressed_index'),
